...
class BotCfg(BaseModel):
    ...
    max_orderbook_levels: PositiveInt = 20
    max_staleness_sec: PositiveInt = 2
    latency_warn_ms: PositiveInt = 800
    spread_min_bps: D = D("0.0008")     # 0.08%
    gain_min_usdt: D = D("5")
    ...
    cb_max_negative_streak: PositiveInt = 3
    cb_cooldown_sec: PositiveInt = 5
...
DEFAULT_CFG = AppCfg(
    ...
    commissioni={
        ...
        "Bitfinex":FeeCfg(acquisto=D("0.002"), vendita=D("0.001")),
...
EXCHANGE_OB = {
    ...
    "Bitfinex": ("https://api-pub.bitfinex.com/v2/book/{}/P0?len={}", "bitfinex"),
...
async def parse_bitfinex_ob(data: List[List[float]]):
    bids, asks = [], []
    for p, c, a in data:
        if a > 0:
            bids.append([float(p), float(a)])
        elif a < 0:
            asks.append([float(p), float(abs(a))])
...
# ======================
FEE_CACHE: Dict[str, Dict[str, Any]] = {}  # {"Binance": {"ts": ..., "maker": D, "taker": D}}
...
async def fees_binance(session: aiohttp.ClientSession) -> Tuple[D, D]:
    ...
    # senza symbol ritorna molte coppie; per velocità chiediamo 1 simbolo comune
    symbol = "BTCUSDT"
    ...
    async with session.get(url, headers=headers, timeout=10) as r:
        data = await r.json()
    # risposta: [{"symbol":"BTCUSDT","makerCommission":"0.001","takerCommission":"0.001"}]
...
# --- Bitfinex: POST /v2/auth/r/summary (auth)
def _bitfinex_sign(secret: str, path: str, nonce: str, body: str) -> str:
    raw = ("bfx" + path + nonce + body).encode()
    return hmac.new(secret.encode(), raw, hashlib.sha384).hexdigest()

async def fees_bitfinex(session: aiohttp.ClientSession) -> Tuple[D, D]:
    if _cache_ok("Bitfinex", CFG.bot.fee_cache_sec):
        e = FEE_CACHE["Bitfinex"]; return e["maker"], e["taker"]
    key = os.getenv("BITFINEX_KEY"); secret = os.getenv("BITFINEX_SECRET")
    if not key or not secret:
        return await get_fees_fallback("Bitfinex")
    base = "https://api.bitfinex.com"; path = "/v2/auth/r/summary"
    url = base + path
    nonce = str(int(time.time()*1000)); body = "{}"
    sig = _bitfinex_sign(secret, path, nonce, body)
    headers = {
        "bfx-nonce": nonce, "bfx-apikey": key, "bfx-signature": sig,
        "content-type": "application/json",
    }
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as sess2:
        async with sess2.post(url, headers=headers, data=body) as r:
            data = await r.json()
    # Estrarre preferibilmente fee "Exchange"
    mk = tk = None
    if isinstance(data, dict) and isinstance(data.get("fees"), list):
        for blk in data["fees"]:
            t = str(blk.get("type","")).lower()
            if "exchange" in t:
                mk = Dsafe(blk.get("maker_fee","0.001"))
                tk = Dsafe(blk.get("taker_fee","0.002"))
                break
        if mk is None and data["fees"]:
            mk = Dsafe(data["fees"][0].get("maker_fee","0.001"))
            tk = Dsafe(data["fees"][0].get("taker_fee","0.002"))
    if mk is None or tk is None:
        return await get_fees_fallback("Bitfinex")
    _put_cache("Bitfinex", mk, tk); return mk, tk
...
async def get_fees(session: aiohttp.ClientSession, exchange: str) -> Tuple[D, D]:
    try:
        ...
        if exchange == "Bitfinex":  return await fees_bitfinex(session)
...
def spread_ok(best_ask: D, best_bid: D, bot: BotCfg) -> bool:
    return best_ask > 0 and best_bid > 0 and (best_bid - best_ask) / best_ask >= bot.spread_min_bps
...
# Circuit breaker
# ======================
CB_STATE: Dict[str, Dict[str, Any]] = {asset: {"streak": 0, "until": 0.0} for asset in CFG.symbols}
    ...
    for asset, by_ex in CFG.symbols.items():
        ...
        for (exg, sym), res in zip(tasks.items(), results):
            if isinstance(res, Exception):
                ...
                continue
            bids, asks, lat = res
            books[exg] = {"symbol": by_ex[exg], "bids": bids, "asks": asks, "latency_ms": lat}
        ...
        if not spread_ok(best_ask, best_bid, bot):
            cb_note(asset, success=False, bot=bot)
            logger.info(f"{asset}: spread insuff ({(((best_bid-best_ask)/best_ask)*100):.4f}%)")
            continue
        ...
        # per semplicità: compra = taker sul lato ask? Dipende da esecuzione.
        # Qui usiamo approccio prudente: compra lato ask come TAKer, vendi lato bid come TAKer.
...
# ======================
async def main():
    logger.info("== avvio arbitraggio async v5 (fee realtime + guardrail) ==")
